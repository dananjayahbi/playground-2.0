import os
from tkinter import Tk, Label, Button, Scale, Entry, StringVar, filedialog, Toplevel, Canvas, HORIZONTAL, VERTICAL, IntVar, OptionMenu
...
class FacebookPostGenerator:
    def __init__(self, root):
        ...
        Label(root, text="Background Blur Factor:").grid(row=18, column=1, sticky="w")
        Scale(root, from_=0, to=20, orient=HORIZONTAL, variable=self.background_blur_factor, command=self.update_preview).grid(row=18, column=2)

        Button(root, text="Generate Post", command=self.generate_post).grid(row=19, column=1, columnspan=2, pady=10)
